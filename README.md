...
各版本正式稳定版：`v1.1.3` `v1.2.2` `v1.3.0`

注意`v1.0.0` `v1.0.1` `v1.2.0`版本存在较严重的bug，不要下载。

### 1.3.0更新内容！
- **1**：现在支持在`cfg.json`中修改主题色！
- **2**：修复了“已知相对湿度”模式
- **3**：秉持公式插件化的理念，彻底重写了计算代码，进一步优化了计算算法，且更方便维护！
- **4**：增加了大量注释，方便维护&fork


## 批量计算使用方法
- **1**：在主页面选择计算模式，选择单位。
- **2**：将数据中的干球温度拷入example.xlsx中的A列，根据模式将露点/湿球/相对湿度拷入B列，大气压强拷入C列。
- **3**：点击批量计算，静待即可！☕
### 注意事项❗
